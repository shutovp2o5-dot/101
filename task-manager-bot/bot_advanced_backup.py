...
(WAITING_TASK_TITLE, WAITING_TASK_COMMENT, WAITING_TASK_PROJECT, WAITING_TASK_DEADLINE, WAITING_TASK_REMINDER, WAITING_TASK_RECURRENCE) = range(6)
(WAITING_PROJECT_NAME, WAITING_PROJECT_TYPE, WAITING_PROJECT_TARGET_TASKS, WAITING_PROJECT_CATEGORY) = range(6, 10)
(WAITING_TASK_COMPLETE_CONFIRM, WAITING_TASK_RESCHEDULE) = range(10, 12)
(WAITING_EDIT_TASK_SELECT, WAITING_EDIT_FIELD_SELECT, WAITING_EDIT_TITLE, WAITING_EDIT_COMMENT, WAITING_EDIT_PROJECT, WAITING_EDIT_DEADLINE, WAITING_EDIT_REMINDER, WAITING_EDIT_RECURRENCE) = range(12, 20)
(WAITING_EDIT_PROJECT_TARGET_TASKS, WAITING_PROJECT_COMPLETE_CONFIRM, WAITING_EDIT_PROJECT_NAME) = range(20, 23)
...
def normalize_voice_text(text: str) -> str:
    ...
    replacements = [
        ...
        (r'\b(\d{1,2})\s*часов\s*(\d{1,2})\b', r'\1:\2'),
        (r'\b(\d{1,2})\s*час\s*(\d{1,2})\b', r'\1:\2'),
        (r'\b(\d{1,2})\s*ч\s*(\d{1,2})\b', r'\1:\2'),
        (r'\b(\d{1,2})\s*часа\s*(\d{1,2})\b', r'\1:\2'),
...
def add_user_project(user_id: str, project_name: str, category: str = None, project_type: str = None, target_tasks: int = None):
    ...
    for pattern, priority in deadline_patterns:
        ...
        if match:
            ...
            if not has_time_of_day and 'в' not in found_text.lower() and not re.search(r'\d{1,2}[:.]\d{2}', found_text):
                ...
                # Более гибкий паттерн для поиска времени
                time_patterns = [
                ...
                for time_pattern in time_patterns:
                    ...
                    if time_after_match:
                        ...
                        if not time_text.startswith('в') and not re.search(r'\b(утра|дня|вечера|ночи)\b', time_text):
                            extended_text = found_text + ' в ' + time_text
                        else:
                            extended_text = found_text + ' ' + time_text
                        # Правильно вычисляем позицию конца расширенного текста
                        extended_end = match.end() + time_after_match.end()
                        print(f"Найдено время после даты: '{time_text}', расширенный текст: '{extended_text}'")
                        break
...
async def add_project_target_tasks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Сразу завершаем создание проекта без категории
    project_type = context.user_data.get('project_type', 'project')
    user_id = update.effective_user.id
    add_user_project(str(user_id), project_name, None, project_type, target_tasks)
